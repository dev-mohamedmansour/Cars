<?php
    ...
    $governorates = $dbAction -> select("*", "governorates")
         -> getAll();
    echo "<form  method='post' style='margin: 10px; padding: 10px; border: 2px solid #0dcaf0; border-radius: 10px;'>";
    ...
    if (isset($_POST[ 'submit' ])) {
        $selectGovernorate = $dbAction -> select("id", "governorates")
             -> where("governorate_name", " = ", "$_POST[governorate]")
             -> getRow();
        $governorateId = $selectGovernorate[ 'id' ];
        $cities = $dbAction -> select("city_name", "cities")
        ...
        echo "<h2 style='color: red'> Please select a governorate. </h2>";
    }
    //        echo "<pre>";
    //        var_dump($_POST['governorate']);
